...
{    
    ...
    {
        uint256 ret_index_val = (hashindex >> DV_UPPER_TOKEN_MASK_BIT_COUNT) & INVALID_INDEX64;
        return uint64(ret_index_val);
    }

    function token_from_hash(uint256 hashindex) internal pure returns(uint256)
    {
        return hashindex & DV_MAX_TOKEN_VALUE;
    }

    function list_count(DVBasicOwnership storage listobj, uint256 parent_token) internal view returns(uint64)
    {
        return listobj.array_sizes[parent_token];
    }


    function clear(DVBasicOwnership storage listobj, uint256 parent_token) internal
    {        
        listobj.array_sizes[parent_token] = uint64(0);
    }
    
    function get(DVBasicOwnership storage listobj, uint256 parent_token, uint64 index) internal view returns(uint256)
    {
    ...
    function set(DVBasicOwnership storage listobj, uint256 parent_token, uint64 index, uint256 value) internal
    {
        uint256 tokenhash = token_index_hash(parent_token, index);
    ...
    /// Removes the peek object from list queue
    function dequeue(DVBasicOwnership storage listobj, uint256 parent_token) internal returns(uint256)
    {
     ...
     */
    function insert(DVBasicOwnership storage listobj, uint256 parent_token, uint256 newtokenID) internal
    {
        uint64 _count = listobj.array_sizes[parent_token];
        uint256 hashindex = token_index_hash(parent_token, _count);
    ...
    /// Returns the new size, and the success flag
    function remove(
        DVBasicOwnership storage listobj,
    ...
    function swap_elements(DVBasicOwnership storage listobj, uint256 parent_token, uint64 index0, uint64 index1) internal
    {
        uint64 _count = listobj.array_sizes[parent_token];        
        require((index0 < _count) && (index1 < _count) && (index0 != index1), "Wrong element indexes");
...
{    
    function token_index_hash(uint256 token_id, uint64 index) internal pure returns(uint256)
    {
        return DVBasicOwnershipUtil.token_index_hash(token_id, index);
    }

    function index_from_hash(uint256 hashindex) internal pure returns(uint64)
    {
        return DVBasicOwnershipUtil.index_from_hash(hashindex);
    }

    function token_from_hash(uint256 hashindex) internal pure returns(uint256)
    {
        return DVBasicOwnershipUtil.token_from_hash(hashindex);
    }

    function list_count(DVOwnershipArray storage listobj, uint256 parent_token) internal view returns(uint64)
    {
        return listobj.array_sizes[parent_token];
    }
    ...
    function get(DVOwnershipArray storage listobj, uint256 parent_token, uint64 index) internal view returns(uint256)
    {
        uint256 tokenhash = DVBasicOwnershipUtil.token_index_hash(parent_token, index);
        return listobj.ownership[tokenhash];
    }


    function set(DVOwnershipArray storage listobj, uint256 parent_token, uint64 index, uint256 value) internal
    {
        uint256 tokenhash = DVBasicOwnershipUtil.token_index_hash(parent_token, index);
        listobj.ownership[tokenhash] = value;
    ...
    function dequeue(DVOwnershipArray storage listobj, uint256 parent_token) internal returns(uint256)
    {
        ...
        // extract element
        uint256 hashindex = DVBasicOwnershipUtil.token_index_hash(parent_token, _count);        
        uint256 value_token = listobj.ownership[hashindex];
     ...
     * Returns the index of the last inserted element
     */
    function insert(DVOwnershipArray storage listobj, uint256 parent_token, uint256 newtokenID) internal
    {
        uint64 _count = listobj.array_sizes[parent_token];
        uint256 hashindex = DVBasicOwnershipUtil.token_index_hash(parent_token, _count);
        listobj.ownership[hashindex] = newtokenID;
    ...
    function _remove_internal(
        DVOwnershipArray storage listobj,
        uint256 parent_token,
        uint64 index,
        uint64 count,
        uint256 hashindex0
    ) internal
    {
        ...
        if(index < new_count)
        {
            ...
            uint256 hashindex1 = DVBasicOwnershipUtil.token_index_hash(parent_token, new_count);
        ...
        uint64 index) internal returns (bool)
    {
        ...
        uint256 hashindex0 = DVBasicOwnershipUtil.token_index_hash(parent_token, index);
        ...
        uint256 exit_tokenID) internal returns (bool)
    {
        ...
        uint256 hashindex0 = DVBasicOwnershipUtil.token_index_hash(parent_token, ref_index - 1);
        listobj.index_ref[exit_tokenID] = 0; // clear index
    ...
    function swap_elements(DVOwnershipArray storage listobj, uint256 parent_token, uint64 index0, uint64 index1) internal
    {
        uint64 _count = listobj.array_sizes[parent_token];        
        require((index0 < _count) && (index1 < _count) && (index0 != index1), "Wrong element indexes");

        uint256 hashindex0 = DVBasicOwnershipUtil.token_index_hash(parent_token, index0);
        uint256 hashindex1 = DVBasicOwnershipUtil.token_index_hash(parent_token, index1);
