...
contract StarGovController is DVControllable
{
    ...
    DVAssetOwnership private _star_ownership_inventory;
    ...
    constructor(address gov_contract_addr, address star_db_addr, uint32 minimum_sponsorship_prestige_level)
    {
    ...
    {     
        if(star_projectID <= DV_DEFAULT_TOKEN_ID64) return address(0);
    ...
    {
        uint256 u_maintain_addr = DVF_ADDRtoU256(maintainer);
    ...
    {
        if(operator == address(0) || star_projectID <= DV_DEFAULT_TOKEN_ID64) return false;
