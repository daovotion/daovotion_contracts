...
import "../util/dvcommon.sol";
import "openzeppelin-contracts/contracts/utils/Address.sol";
import "openzeppelin-contracts/contracts/security/ReentrancyGuard.sol";

uint256 constant DV_NUCLEUS_TOKEN_PRICE = 0.01 ether;

error ErrDVNucleus_NotEnoughPaymentToMintNucleus(address receiver, uint256 payment, uint256 required_price);

error ErrDVNucleus_InsufficientNucleusBalance(address owner, uint256 required_balance);

error ErrDVNucleus_InvalidParams();

contract DVNucleusToken is Ownable, ReentrancyGuard 
{
    ...
    constructor()
    {
        _nucleus_token_price = DV_NUCLEUS_TOKEN_PRICE;
        _total_supply = 0;
    }
    ...
    /**
     *  Called from DAOVOTION.
     *  This only could mint 1 nucleus token at a time. Extra payment would be absorved by DAOVOTION
     */
    function nucleus_token_mint(address receiver, uint256 payment) external onlyOwner nonReentrant
    {
        ...
        {
            revert ErrDVNucleus_NotEnoughPaymentToMintNucleus(receiver, payment, _nucleus_token_price);
        }
    ...
    /**
     * Called from DAOVOTION, for creating projects.
    ...
    function spend_nucleus_token(address spender) external onlyOwner nonReentrant
    {
        ...
        {
            revert ErrDVNucleus_InvalidParams();
        }
        
        if(_nucleus_token_balance[spender] < 1)
        {
            revert ErrDVNucleus_InsufficientNucleusBalance(spender, 1);
        }
