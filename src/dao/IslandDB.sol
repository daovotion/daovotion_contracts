...
contract IslandDB is DVControllable, ReentrancyGuard 
{
    ...
    DVOwnershipArray private _island_name_registry;

    /// Maps islandID with address
    DVAssetOwnership private _islands_ownership;

    uint256 _new_islandID;

    constructor()
    {
        _new_islandID = DV_DEFAULT_TOKEN_ID;
    }

    function _check_island_owner(address operator, uint256 islandID) internal view
    {
        if(operator == address(0) || islandID <= DV_DEFAULT_TOKEN_ID)
        {
            revert ErrDAO_InvalidParams();
        }

        IslandInfo storage pinfo = _islands_info[islandID];
        if(pinfo.reputation == 0 || pinfo.star_projectID <= DV_DEFAULT_TOKEN_ID64 || pinfo.planetID <= DV_DEFAULT_TOKEN_ID)
        {
    ...
    {        
        ...
        if(DVF_IsNullName(pmeta.name) == true) return;
    ...
    {
        if( owner == address(0) || 
            star_projectID <= DV_DEFAULT_TOKEN_ID64 ||
    ...
    {     
        if(islandID <= DV_DEFAULT_TOKEN_ID) return address(0);
    ...
    {     
        if(islandID <= DV_DEFAULT_TOKEN_ID || pk_owner == address(0)) return false;
