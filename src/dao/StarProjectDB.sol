...
import "../util/DVControllable.sol";
...
uint64 constant DV_DEFAULT_TOKEN_ID64 = 1;// A reference value that is not used and could indicates a null.
...
contract StarProjectDB is DVControllable, IStarProjectDB
{
    using DVOwnershipArrayUtil for DVOwnershipArray;
     ...
     */
    DVOwnershipArray private _star_sanctions;
    ...
    DVOwnershipArray _star_collections;
    
    constructor()
    {
        _starprojectID = DV_DEFAULT_TOKEN_ID64;
    }
    ...
    {
        if(_projectID <= DV_DEFAULT_TOKEN_ID64 || _projectID > _starprojectID)
        {
    ...
    function _checkValidNotOblivion(uint64 _projectID) internal view
    {
        ...
        if(_prestige_info.promotion_status == DV_STATUS_GUILTY && _prestige_info.prestige_level == 0)
        {
    ...
    {
        if(DVF_IsNullName(star_name) == true) return 0;
    ...
    function _initialize_project(uint64 projectID) internal
    {
        ...
        pricing_info.insurance_risk_threshold = DV_MIN_INSURANCE_RISK_THRESHOLD;        
    ...
    ) onlyOwner external returns(uint64)
    {
        ...
        if(DVF_IsNullName(project_info.name) == false)
        {
    ...
    {
        if(DVF_IsNullName(newname) == false)
        {
        ...
        // erase old name
        if(DVF_IsNullName(metainfo.name) == false)
        {
    ...
    function get_star_collection_type(uint64 star_projectID) public view returns(uint32)
    {
        ...
        if(status == DV_STATUS_GUILTY)
        {
            list_type = STAR_LIST_GUILTY;
        }
        else if(status == DV_STATUS_REDEMPTION)
        {
            list_type = STAR_LIST_REDEMPTION;
        }
        else if(status == DV_STATUS_PROMOTING)
        {
            list_type = STAR_LIST_PROMOTING;
        }
        else if(status == DV_STATUS_UNACCREDITED)
        {
    ...
    {
        StarPrestige storage prestige_info = _star_prestige[star_projectID];
        return prestige_info.promotion_status == DV_STATUS_UNACCREDITED || prestige_info.promotion_status == DV_STATUS_PROMOTING;
    }

    function is_star_in_oblivion(uint64 star_projectID) public view returns(bool)
    {
        StarPrestige storage prestige_info = _star_prestige[star_projectID];
        return prestige_info.promotion_status == DV_STATUS_GUILTY && prestige_info.prestige_level == 0;
    }
    ...
    {
        if(prestige_info.promotion_status == DV_STATUS_GUILTY || prestige_info.promotion_status == DV_STATUS_REDEMPTION)
        {
    ...
    {        
        ...
        if((prestige_info.promotion_status & DV_STATUS_SPONSORSHIP_BONUS) != 0)
        {
            earned_periods++;
            if((prestige_info.promotion_status & DV_STATUS_SPONSORSHIP_EXTRA_BONUS) != 0)
            {
                earned_periods++;
            }
        }        
        if((prestige_info.promotion_status & DV_STATUS_HONORABLE_BONUS) != 0)
        {
            earned_periods++;
        }
        ...
        if(prestige_info.prestige_level >= minimum_sponsorship_prestige_level &&
           (prestige_info.promotion_status & DV_STATUS_PARDONED) == 0)
        {
            ...
            if(sponsorID != uint64(0))
            {
                ...
                if(sponsor_prestige.promotion_status > 0 &&
                   (sponsor_prestige.promotion_status & DV_STATUS_SPONSORSHIP_BONUS) != 0)
                {
                    sponsor_prestige.promotion_status |= DV_STATUS_SPONSORSHIP_EXTRA_BONUS;
                }
    ...
    {        
        ...
        if(sponsor_prestige.promotion_status > 0)
        {
            ...
            sponsor_prestige.promotion_status = DV_STATUS_ACCREDITED | DV_STATUS_PARDONED;
        }
     ...
     * Returns the tuple StarIndemnization telling the indebted compensation, the sentence hash and the ammount to be paid from DAOVOTION to the indeminzed party.
    ...
    ) external onlyOwner validStarProject(star_projectID) returns(StarIndemnization memory)
    {
        ...
        uint256 safe_insurance_amount = DVF_PERCENTAJE(revenue.insurance_vault, insurance_risk_threshold);
        ...
        // condemned_prestige
        condemned_prestige.promotion_status = DV_STATUS_GUILTY;
    ...
    function star_can_apply_for_redemption(uint64 star_projectID, uint256 redemption_fee) public view returns(bool)
    {
        ...
        if(prestigeinfo.promotion_status != DV_STATUS_GUILTY || prestigeinfo.prestige_level < 1) return false;
    ...
    {
        StarPrestige storage prestigeinfo = _star_prestige[star_projectID];
        if(prestigeinfo.promotion_status != DV_STATUS_GUILTY || prestigeinfo.prestige_level < 1)
        {
        ...
        prestigeinfo.promotion_status = DV_STATUS_REDEMPTION;        
    ...
    function redeem_star_from_list(uint64 index, uint grace_period_duration) external onlyOwner
    {
        ...
        assert(prestige.promotion_status == DV_STATUS_REDEMPTION);

        prestige.promotion_status = DV_STATUS_PARDONED | DV_STATUS_ACCREDITED;        
        ...
        if(elapsed_time > 0)
        {
            ...
            if((periods_lost + DV_ACCREDITED_AGE_INIT) >= prestige.prestige_level)
            {
                // set the minimum
                prestige.prestige_level = DV_ACCREDITED_AGE_INIT;
            }
            else
            {
    ...
    ) external onlyOwner validStarProject(star_projectID) returns(StarIndemnization memory)
    {
        ...
        if(prestige.promotion_status != DV_STATUS_GUILTY)
        {
    ...
    {
        StarPrestige storage prestige = _star_prestige[star_projectID];
        if(prestige.promotion_status == DV_STATUS_GUILTY || prestige.promotion_status == DV_STATUS_REDEMPTION)
        {
            revert ErrDAO_CannotCondemGuiltyStar(star_projectID);
        }

        uint collection_type = prestige.promotion_status == DV_STATUS_PROMOTING ? STAR_LIST_PROMOTING:
        (prestige.promotion_status == DV_STATUS_UNACCREDITED ? STAR_LIST_NEBULOUS : STAR_LIST_ACCREDITED );
        ...
        /// Set guilty
        prestige.promotion_status = DV_STATUS_GUILTY;
    ...
    {
        StarPrestige storage prestigeinfo = _star_prestige[star_projectID];
        if(prestigeinfo.promotion_status != DV_STATUS_UNACCREDITED) return false;
    ...
    {
        StarPrestige storage prestigeinfo = _star_prestige[star_projectID];
        if(prestigeinfo.promotion_status != DV_STATUS_UNACCREDITED)
        {
        ...
        prestigeinfo.promotion_status = DV_STATUS_PROMOTING;
    ...
    {
        prestigeinfo.promotion_status = DV_STATUS_ACCREDITED;
        prestigeinfo.prestige_level = DV_ACCREDITED_AGE_INIT;
        prestigeinfo.last_promotion_timestamp = block.timestamp;        
    ...
    function promote_star_from_list(uint64 index) external onlyOwner
    {
        ...
        assert(prestige.promotion_status == DV_STATUS_PROMOTING);
    ...
    function _promote_star_ex(uint64 star_projectID) internal
    {
        ...
        uint collection_type = prestigeinfo.promotion_status == DV_STATUS_PROMOTING ? STAR_LIST_PROMOTING:
        (prestigeinfo.promotion_status == DV_STATUS_UNACCREDITED ? STAR_LIST_NEBULOUS : STAR_LIST_ACCREDITED );
    ...
    ) external onlyOwner validStarProject(star_projectID) validStarProject(sponsorID)
    {
        ...
        if((sponsorprestige.promotion_status <= 0) ||
           ...
           (sponsorprestige.promotion_status & DV_STATUS_PARDONED) != 0 ||
        ...
        // reward as sponsor
        sponsorprestige.promotion_status |= DV_STATUS_SPONSORSHIP_BONUS;
