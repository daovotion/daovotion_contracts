...
import "../util/DVCalcLib.sol";
import "../util/DVControllable.sol";
import "../util/DVAssetOwnership.sol";
...
struct IslandPricingParams
{
    ...
    /// DAOVOTION Tax rate, calculated over raw commission before taxes.
    uint256 daovotion_tax_rate;
}
...
struct IslandPriceCalculation
{
    ...
    uint256 daovotion_tax_contribution;
}

/**
 * Planets never dissapear.
 */
contract PlanetDB is DVControllable, ReentrancyGuard 
{
    using DVOwnershipArrayUtil for DVOwnershipArray;
    using DVAssetOwnershipUtil for DVAssetOwnership;
    using DVBasicOwnershipUtil for DVBasicOwnership;
    ...
    DVOwnershipArray private _planet_name_registry;
    
    /// Maps planetShareID with address
    DVAssetOwnership private _planet_share_ownership;

    /**
     * Maps planets associated to star projects
     */
    DVBasicOwnership private _star_planets_map;
    ...
    constructor()
    {
        _new_planetID = DV_DEFAULT_TOKEN_ID;
    }
    ...
    {
        if(DVF_IsNullName(name) == true) return 0;
    ...
    function _unregister_planet_name(uint256 planetID) internal
    {
        ...
        if(DVF_IsNullName(pinfo.name) == true) return;
    ...
    /**
     * If new_name == DV_NULL_NAME it doesn't register the project
     */
    function _change_planet_name(uint planetID, bytes32 new_name) internal
    {
        ...
        if(DVF_IsNullName(new_name) == false)
        {
    ...
    ) internal returns(uint256)
    {
        ...
        PlanetInfo storage pinfo = _planets_info[planetID];
        if(pinfo.star_projectID <= DV_DEFAULT_TOKEN_ID64)
        {
    ...
    /**
     * This method is called by DAOVOTION
     */
    ...
    {     
        if(shareID <= DV_DEFAULT_TOKEN_ID) return address(0);
    ...
    {     
        if(shareID <= DV_DEFAULT_TOKEN_ID || pk_owner == address(0)) return false;
    ...
    /**
     * Remarks: selling_price is optional. The actual payment from DAOVOTION, can be 0.
     */
    function calc_island_price(
        uint256 planetID,
        uint256 selling_price, // the actual payment from DAOVOTION, can be 0
        IslandPricingParams memory pricing_params
    ) public view returns(IslandPriceCalculation memory)
    {
        PlanetInfo storage pinfo = _planets_info[planetID];
        
        uint256 curvefloorprice = DVCalcLib.curve_floor_price(StarIslandBondingCurve({
        ...
        uint256 insurance = DVF_PERCENTAJE(rawcommision, pricing_params.insurance_rate);
        uint256 daovotion_tax = DVF_PERCENTAJE(rawcommision, pricing_params.daovotion_tax_rate);

        return IslandPriceCalculation({
            island_sales_price: (curvefloorprice + rawcommision),
            planet_commission: (rawcommision - insurance - daovotion_tax),
            insurance_contribution: insurance,
            daovotion_tax_contribution: daovotion_tax
        });
    }
    ...
    /**
     * Commission is calculated by DAOVOTION.
     * PRE: DAOVOTION should check if Star project allows free minting.
     */
    ...
    ) external onlyOwner
    {
        ...
        /*
        --- DAOVOTION should check if Star project allows free minting.
    ...
    ) public view returns(uint256, address)
    {
        ...
        uint256 portion_commissions = DVF_PERCENTAJE(accumulated_commissions, pshareinfo.share_percentaje);
    ...
    ) external onlyOwner returns(uint256, uint64)
    {
        ...
        uint256 portion_commissions = DVF_PERCENTAJE(pinfo.liberated_commissions, pshareinfo.share_percentaje);
