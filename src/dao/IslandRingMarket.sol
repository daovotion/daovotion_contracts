...
contract IslandRingMarket is DVRingMarket
{
    mapping(uint64 => RingGenRange) internal _rings_x_project;
    mapping(uint256 => uint32) internal _island_rings_result; 

    constructor(address equityListenerAddr, uint32 handleCode)
    DVRingMarket(5, equityListenerAddr, handleCode)
    {
    }
