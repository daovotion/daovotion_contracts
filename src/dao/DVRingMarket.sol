 ...
 */
abstract contract DVRingMarket is DVControllable, ReentrancyGuard
{
    ...
    DVAssetOwnership internal _ring_token_owners;

    DVAssetOwnership internal _equity_owners;

    constructor(uint32 groupSize, address equityListener, uint32 handleCode)
    {
        _group_size = groupSize;
        _equity_listener = IRingMarketListener(equityListener);
        _listener_handle_code = handleCode;
        _new_equityID = DV_DEFAULT_TOKEN_ID;
    }
    ...
    {
        if(equityID <= DV_DEFAULT_TOKEN_ID) return false;
    ...
    {
        address targetaddr = msg.sender;
        if(tokenID == 0 || equityID <= DV_DEFAULT_TOKEN_ID || targetaddr == address(0))
        {
            revert ErrDAO_InvalidParams();
        }
