...
/**
 * In DAOVOTION, Only through Meta Islands DAO members could pariticpate in governance decisions.
 * 
 */
contract StarGovernance is DVControllable
{    
    ...
    DVGovernanceTimeParams private _quorum_expiration_time;
     ...
     */
    DVBasicOwnership private _island_vote_registry;

    /**
     * Proposals to be approved in voting sessions.
     * Contains the ID of the voting sessions
     */
    DVBasicOwnership private _star_voting_proposals;

    /**
     * Approved tasks with budget already reserved to them
     */
    DVBasicOwnership private _star_approved_tasks;
    ...
    constructor()
    {
        _new_taskID = DV_DEFAULT_TOKEN_ID;
        _new_sessionID = DV_DEFAULT_TOKEN_ID;
    ...
    ) internal pure returns(eVotingState)
    {
        ...
        uint minimum_portion_count = DVF_PERCENTAJE(island_population, quorum_rate);
    ...
    function _execute_quorum(uint256 sessionID, bool deadline_reached) internal returns(eVotingState, eDeliberationKind)
    {
        ...
        assert(taskID > DV_DEFAULT_TOKEN_ID && taskID <= _new_taskID);
        StarTaskInfo storage taskinfo = _star_tasks_registry[taskID];
    ...
    {
        if(sessionID > _new_sessionID || sessionID <= DV_DEFAULT_TOKEN_ID)
        {
