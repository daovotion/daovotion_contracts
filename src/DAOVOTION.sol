...
uint32 constant DV_RING_MARKET_HANDLE_CODE = 5;


contract DAOVOTION is ReentrancyGuard, Ownable, IRingMarketListener
{    
    ...
    ///  ********** Global parameters **************** ///    
    DAOVOTIONParams private _global_params;
    ///  ********** **************************** **************** ///
    ...
    DVNucleusToken private _nucleus_token_contract;
    ...
    ///  ********** *********General DAOVOTION budget ************ ///    
    uint256 private _daovotion_budget;
    ...
    {        
        ...
        _global_params.minimum_sponsorship_prestige_level = DV_AGE_FOR_SPONSORSHIP;
        ...
        _punishment_sentenceID = DV_DEFAULT_TOKEN_ID;

        _star_governance_contract = new StarGovernance();
        _star_db_contract = new StarProjectDB();
        ...
        _planets_db_contract = new PlanetDB();
        _islands_db_contract = new IslandDB();
        _nucleus_token_contract = new DVNucleusToken();
        _randao_contract = new DVRANDAO();

        // Ring market will notify DAOVOTION when completing rings group        
        _rings_market_contract = new IslandRingMarket(address(this), DV_RING_MARKET_HANDLE_CODE);
    }
    ...
    function get_daovotion_params() public view returns(DAOVOTIONParams memory)
    {
        return _global_params;
    }

    function config_daovotion_params(DAOVOTIONParams memory params) external onlyOwner nonReentrant
    {
    ...
    ) external nonReentrant returns(uint256, uint256)
    {
        ...
        PlanetEntryParams memory planet_params = PlanetEntryParams({
            ...
            star_prestige: DV_PIONEER_SPONSOR_PLANET_CURVE,
    ...
    {
        address target = msg.sender;
        if(target == address(0) || star_projectID <= DV_DEFAULT_TOKEN_ID64)
        {
            revert ErrDAO_InvalidParams();
        }
    ...
    {
        if(target == address(0) || planetID <= DV_DEFAULT_TOKEN_ID)
        {
            revert ErrDAO_InvalidParams();
        }

        PlanetInfo memory pinfo = _planets_db_contract.get_planet_info(planetID);
        if(pinfo.star_projectID <= DV_DEFAULT_TOKEN_ID64 || pinfo.star_prestige == 0)
        {
            revert ErrDAO_InvalidParams();
        }
    ...
    ) internal returns(uint256)
    {
        StarCommissionIncome memory income_balance = _star_db_contract.contribute_star_commissions(
            ...
            StarCommissionIncome({
                ...
                tax_reserve_payment: price_calculation.daovotion_tax_contribution,
        ...
        uint256 star_budget_contribution = price_calculation.island_sales_price -
                                           ...
                                           price_calculation.daovotion_tax_contribution -
        ...
        _daovotion_budget += price_calculation.daovotion_tax_contribution;
    ...
    ) external payable nonReentrant returns(uint256)
    {
        ...
        IslandPriceCalculation memory price_calculation = _planets_db_contract.calc_island_price(
            ...
            IslandPricingParams({
                ...
                daovotion_tax_rate: _global_params.global_tax_rate
            })
        );
    ...
    {
        if(planet_shareID <= DV_DEFAULT_TOKEN_ID)
        {
            revert ErrDAO_InvalidParams();
        }
    ...
    {
        if(star_projectID <= DV_DEFAULT_TOKEN_ID64 || target == address(0) || payment == 0)
        {
            revert ErrDAO_InvalidParams();
        }
        // check project
    ...
    {
        _daovotion_budget += _star_db_contract.upgrade_prestige_level(
            star_projectID, 
            _global_params.minimum_sponsorship_prestige_level,
    ...
    function trigger_island_randao_campaign(uint256 islandID) external payable nonReentrant returns(uint256)
    {
        ...
        if(clientaddr == address(0) || payment == 0 || islandID <= DV_DEFAULT_TOKEN_ID)
        {
            revert ErrDAO_InvalidParams();
        }
    ...
    {
        address clientaddr = msg.sender;
        if(clientaddr == address(0) || islandID <= DV_DEFAULT_TOKEN_ID)
        {
            revert ErrDAO_InvalidParams();
        }
    ...
    {        
        if(handle_code == DV_RING_MARKET_HANDLE_CODE)
        {
