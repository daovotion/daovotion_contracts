...
contract DVRANDAOTest is Test {
	...
	DVOwnershipArray _records_x_island;
	...
	)internal returns(uint8, bytes32, bytes32) // (uint8 parity_v, bytes32 sign_r, bytes32 sign_s)
	{
        ...
        bytes32 eth_digest = ECDSA.toEthSignedMessageHash(circle_island_hash);// adapt to Ethereum signature format
	...
	) internal returns(uint256)
	{
		...
		uint32 rcount = _records_x_island.insert(islandID, new_recordID);
		console.log("-> Island last record Inventory: ", rcount, ".\n");
